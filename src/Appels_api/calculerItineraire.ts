...
export async function exportFichierPDF(listeLivraisons : Livraisons): Promise<{ message: string, data: Blob }> {
    return new Promise(async (resolve, reject) => {
        try { //http://localhost:8080/carte/calculerItineraireCluster
            //http://localhost:8080/carte/calculerItineraireClusterOptimal
            const req = await fetch(`http://localhost:8080/export/pdf  `, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
...
export async function exportFichierXML(listeLivraisons : Livraisons): Promise<{ message: string, data: Blob }> {
    return new Promise(async (resolve, reject) => {
        try { //http://localhost:8080/carte/calculerItineraireCluster
            //http://localhost:8080/carte/calculerItineraireClusterOptimal
            const req = await fetch(`http://localhost:8080/export/xml  `, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
